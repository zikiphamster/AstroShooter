...
const PORTAL_DUR         = 5.0;
let neptuneShipFallVy    = 0;    // downward velocity for ship-on-fire falling animation
...
const CHANGELOG = [
  ...
  { v: 'v1.63.0', date: '9:49 AM, 01 Mar 2026',  title: 'Dialogue Overhaul',         desc: 'All 9 planets now have 2 randomized pre-launch dialogue scripts picked at random on each visit, with varied speaker order and unique content. The Sun features a special intro from Earth Control Tower explaining the mission and hinting at a larger threat. Each boss encounter now opens with a 4-line exchange between the boss and the pilot, rendered over the live game world with a skull avatar in the boss\'s color. Endless Mode is unaffected.' },
  { v: 'v1.63.1', date: '10:35 AM, 01 Mar 2026',  title: 'Boss Entrance Cinematic',   desc: 'When a boss spawns in Story Mode, the battlefield is instantly cleared, the player\'s ship auto-pilots to the left-center of the screen, and the boss slides fully onto screen before the dialogue panel appears. Stars continue scrolling throughout the cinematic.' },
  { v: 'v1.64.0', date: '4:38 PM, 01 Mar 2026',  title: 'The Veil Expanse',          desc: 'After defeating Neptune\'s boss, a cinematic plays: the dying boss reveals a larger conspiracy, a portal tears open, and the pilot is pulled into The Veil Expanse — a new galaxy with 9 harder planets, unique bosses, original dialogue, violet-tinted stars, and purple nebula visuals.' },
  { v: 'v1.64.1', date: '4:56 PM, 01 Mar 2026',  title: 'Neptune Death Polish',      desc: 'The Neptune boss defeat sequence is now fully cinematic: all remaining asteroids explode simultaneously, the boss stays on screen burning with fire particles and screen shake, and the player\'s ship glides back to the left-center before the dialogue panel appears. Galaxy navigation arrows also added to the solar map.' },
  { v: 'v1.64.2', date: '4:56 PM, 01 Mar 2026',  title: 'Difficulty Rebalance',      desc: 'All 18 planets across both galaxies are now easier: asteroids spawn less frequently, move slower, and skew smaller. Planet hazards appear less often and cap at lower simultaneous counts. Several later planets gained an extra life.' },
  { v: 'v1.64.3', date: '5:20 PM, 01 Mar 2026',  title: 'Shop Stat Comparison',      desc: 'Clicking a hull or engine in the shop now enters a preview mode instead of instantly equipping. The attributes panel switches to a side-by-side comparison (current bar vs. preview bar) with colour-coded +/- deltas per stat. A BUY / EQUIP button appears below — click it to confirm, or click the item again to dismiss the preview.' },
  { v: 'v1.64.4', date: '5:24 PM, 01 Mar 2026',  title: 'Veil Boss HP Rebalance',     desc: 'All 9 Veil Expanse bosses have 20% less HP, making encounters feel more manageable. A minimum floor ensures every Veil boss still has more health than Neptune, the toughest Solar System boss.' },
  { v: 'v1.64.5', date: '5:40 PM, 04 Mar 2026',  title: 'Level Complete Cleanup',     desc: 'The "+2 LIVES BONUS" text no longer appears on the level complete screen in Story Mode, where boss kills do not grant extra lives. The bonus text is now exclusive to Endless and Arcade modes.' },
  { v: 'v1.64.6', date: '5:42 PM, 01 Mar 2026',  title: 'Neptune Cinematic Overhaul', desc: 'Reworked the Neptune boss death sequence: the defeated boss now slides to the far-left of the screen, its back ignites with fire particles, and the player is pushed rightward into the portal. Duration extended from 2.5 s to 3.5 s. Veil Expanse planets now each have a unique themed hazard distinct from their Solar System counterparts.' },
  { v: 'v1.64.7', date: '8:00 PM, 02 Mar 2026',  title: 'Veil Unique Hazards',        desc: 'All 9 Veil Expanse planets now have completely original hazard types — none shared with the Solar System set. New dangers include: void_pulse (expanding damage ring), shock_mine (periodic electric burst), phase_rift (moving vertical rift), crystal_spike (fast projectile), plasma_ball (bouncing orb), heat_wave (sweeping column that pushes the player), freeze_field (slow + damage zone), shadow_tendril (homing dark orb), and nova_flash (warning dot → explosion ring).' },
  { v: 'v1.64.8', date: '7:03 PM, 05 Mar 2026', title: 'Next Planet Button Fix',       desc: 'The "Next Planet" button on the level complete screen in Story Mode now correctly advances to the next planet — loading its difficulty settings, triggering the warp launch animation, and playing the intro dialogue before starting gameplay. Previously it returned to the solar map without launching.' },
  { v: 'v1.64.9', date: '7:23 PM, 05 Mar 2026',  title: 'Planet Visual Detail Pass',    desc: 'All planets now have rich surface detail in cutscenes and on the solar map. Earth shows five recognisable landmasses (Americas, Eurasia, Africa, Australia), polar ice caps, and a cloud wisp. Other Solar System planets gained atmospheric bands, craters, canyons, storms, and the Great Red Spot in cutscenes. All 8 non-Aethon Veil Expanse planets received unique themed surface details on both the map and in cutscenes (lava flows, crystal facets, ice fractures, obsidian patches, nova cores, etc.).' },
  { v: 'v1.65.0', date: '7:23 PM, 05 Mar 2026',  title: 'Touch Dialogue Fix',           desc: 'Fixed a touchscreen bug where the joystick and fire zones overlapped with dialogue and cutscene NEXT buttons. Joystick and fire zones are now suppressed during any active overlay (planet intro, boss dialogue, Neptune death cutscene, tutorial). Tapping NEXT/FIGHT/LAUNCH during boss dialogue and the Neptune death cutscene now correctly advances the sequence on touch devices.' },
  { v: 'v1.66.0', date: '7:23 PM, 05 Mar 2026',  title: 'Visual Detail Pass',            desc: 'Major visual upgrade across three areas: Planets now display animated cloud layers that orbit the planet surface in real-time — Earth, Venus, gas giants, and atmospheric Veil planets each have unique cloud speeds and tints. Ships now render with 3D hull shading gradients, panel division lines with edge highlights, an engine nozzle with blue glow, a gradient cockpit with glass reflection, and rivet detail dots. Asteroids now feature a radial shading gradient for a spherical look, deterministic crater placement, randomized surface crack angles, and split into two angular shard fragments that spin in opposite directions rather than generic blobs.' },
  { v: 'v1.67.0', date: '8:32 PM, 09 Mar 2026',  title: 'Ship & Planet Visual Overhaul', desc: 'Ships now use a dark navy hull base (#22293d) matching the futuristic spacecraft aesthetic — the player\'s chosen color is used exclusively for V-shaped wing accent stripes, three engine nozzle glows, and cockpit tint. Thruster is redesigned as three parallel oval flame blobs instead of a single triangle. Earth\'s cutscene planet now displays realistic bezier-curve continent outlines for North America, South America, Eurasia, Africa, and Australia instead of simple ellipses.' },
  { v: 'v1.68.0', date: '5:30 PM, 10 Mar 2026',  title: 'Futuristic Ship Architecture',  desc: 'Complete ship structural redesign matching reference futuristic spacecraft. Ships now feature a narrow bezier-curve central fuselage body layered on top of wide swept-back wing shapes — creating the distinct body+wing silhouette of real sci-fi fighters. Nozzle and thruster flame count now scales with engine tier: Standard=1, Afterburner/Pulse=2, Bulwark/Ion=3, Quantum=4, so upgrading your engine visibly adds more thruster nozzles. Hull details include 3D wing shading, diagonal color accent stripes, fuselage panel seams, cockpit dome with glass reflection highlight, and rivet dots at wing/body junctions.' },
  { v: 'v1.69.0', date: '7:32 PM, 10 Mar 2026', title: 'Full 3D Ship Detail Pass',        desc: 'Comprehensive 3D texture and detail upgrade to player ships. Each ship now features: 3D cylindrical engine pods (elongated ellipses with top-lit gradient, arc highlight, edge outline, and decorative groove band) replacing flat nozzle circles; dual forward-facing gun barrels extending from the nose with bevel highlights and muzzle tips; stronger wing panel contrast with darker seam lines plus highlight edges; color accent filled panels on wings (not just stripes) with a brighter leading edge; layered fuselage with outer shell plus inset inner panel and top edge highlight curve; deeper panel seams with matching light highlights; nozzle glow inner bright white core for visual punch; cockpit dark frame ring with dual glass reflections and a color glow ring; and rivets with 3-layer shadow/cap/specular construction for a raised bolt appearance.' },
  { v: 'v1.70.0', date: '1:13 PM, 11 Mar 2026', title: 'Mobile Layout Fix',               desc: 'Fixed multiple UI layout issues on phone-sized screens. The difficulty select screen now switches from a 3-column horizontal layout (858px wide) to a vertical single-column stack when the screen is narrower than 700px, with the title and subtitle repositioned accordingly. Solar map planet rail now uses responsive margins (percentage-based) instead of hardcoded 170px/130px offsets — on portrait phones the full planet range is visible. Galaxy navigation arrows moved from fixed ±200px offsets (which went off-screen on 390px phones) to screen-edge placement. Play Mode and Settings button widths are now clamped to screen width minus padding. Menu subtitle text now compresses to fit narrow screens via maxWidth.' },
  { v: 'v1.71.0', date: '3:13 PM, 17 Mar 2026', title: 'Neptune Cinematic Redesign',       desc: 'The Neptune death / Veil portal sequence is now a live cinematic. Boss instantly explodes with a large particle burst and vanishes (no more 3.5-second dying slide). The full-screen 75% black overlay is removed — the game world stays fully visible throughout. Asteroids continue drifting during all dialogue and portal phases; no new asteroids or power-ups spawn. The player ship catches fire (orange/yellow flames + gray smoke particles trail from the back) and falls off the bottom of the screen under gravity. After clicking "PORTAL ▶", the portal grows on the right side of the screen; the ship re-enters from below and flies into the portal with a smoke trail. After the flash transition, the ship emerges on the far right and slides left into the Veil Expanse. A faint pinkish radial glow tints the background throughout, intensifying slightly during the arrival dialogue.' },
